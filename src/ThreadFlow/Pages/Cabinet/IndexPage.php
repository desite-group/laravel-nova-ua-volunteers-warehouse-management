...
class IndexPage extends AbstractPage
{
    protected function show()
    {
        TextOutgoingMessage::make(
            "Дисклеймер: Це АЛЬФА версія бота ще не має повного функціорналу. На даний момент побудована лише ще не фінальна структура майбутнього функціоналу.")
            ->reply();
    ...
    private function getUserButtons(): array
    {
        ...
        if (!empty($botUser->role->permissions)) {
            ...
            if ($permissions->where('code', 'loading')->first()) {
                $buttons['loading'] = '📦 Завантаження/розвантаження';
            }
